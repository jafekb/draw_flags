...
def download_flag_images(flags: List[Flag], output_dir: Path, rate_limit_seconds: float = 1.0):
    ...
    for i, flag in enumerate(flags):
        ...
        if output_file.exists():
            skipped += 1
            if (i + 1) % 100 == 0:
                print(f"  Progress: {i + 1}/{len(flags)} (successful: {successful}, failed: {failed}, skipped: {skipped})")
            continue
        ...
        for attempt in range(max_retries):
            try:
                ...
                response = requests.get(
                    ...
                    timeout=30,
                    headers={'User-Agent': 'DrawFlags/1.0 (https://github.com/jafekb/draw_flags/; jafek91@gmail.com)'}
                )
...
def generate_embeddings(flags: List[Flag], images_dir: Path, output_file: Path):
    ...
    try:
        ...
        print("Install with: pip install sentence-transformers pillow")
        return
    ...
    for i, flag in enumerate(flags):
        ...
        try:
            ...
            if (i + 1) % 100 == 0:
                print(f"  Progress: {i + 1}/{len(flags)} (successful: {successful}, failed: {failed})")
...
def main():
    ...
    flags_file = Path("backend/data/comprehensive_flags/flags_deduplicated.json")
    ...
    print(f"To use this dataset, update backend/src/flag_searcher.py:")
    print(f"  FLAGS_FILE = Path('backend/data/comprehensive_flags/flags.json')")
