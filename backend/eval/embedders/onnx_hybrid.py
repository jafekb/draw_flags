...
class OnnxHybridExperiment:
    def __init__(self, name: str, onnx_path: str, tokenizer_path: str, weight: float) -> None:
        self.name = name
        self._weight = weight
    ...
    def rank(self, queries: List[str], top_k: int) -> List[List[str]]:
        ...
        for row, raw_query in zip(dense, queries):
            score = row + self._weight * name_overlap_scores(raw_query, self._name_token_sets)
